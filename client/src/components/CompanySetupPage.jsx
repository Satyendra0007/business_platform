...
import {
  ...
  Link, ArrowRight, CheckCircle2, Loader2, X
} from 'lucide-react';
...
function StepDot({ label, active, done, index }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div
        className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
            ...
            : active
            ? 'bg-[#E5A93D] text-[#0A2540] shadow-[0_0_12px_rgba(229,169,61,0.4)]'
            : 'bg-white/10 text-white/40'
        }`}
      >
        {done ? <CheckCircle2 className="h-4 w-4" /> : index + 1}
      </div>
      <span className={`text-[10px] font-semibold uppercase tracking-wider ${active ? 'text-white' : 'text-white/40'}`}>
        {label}
      </span>
    </div>
  );
...
export default function CompanySetupPage() {
  ...
  const [step, setStep] = useState(0); // 0 = basics, 1 = details, 2 = done
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Sync companyId back into the user context + localStorage
  ...
  // ─── STEP CONFIG ─────────────────────────────────────────────────────────────
  const steps = ['Basics', 'Details', 'Done'];
        ...
        <div>
          ...
          <div className="mt-14 space-y-4">
            {[
              ...
              { icon: CheckCircle2, text: 'Admin will review and verify your company before you can list products.' },
        ...
        <div className="w-full max-w-xl">
          ...
          {step === 0 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <button
                onClick={nextStep}
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0A2540] py-4 text-sm font-bold text-white transition hover:bg-[#143a6a] hover:-translate-y-0.5"
              >
          ...
          {step === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setStep(0)}
                  className="rounded-2xl border border-slate-200 px-6 py-4 text-sm font-bold text-slate-600 transition hover:bg-slate-50"
                >
                  Back
                </button>
                <button
            ...
            <div className="space-y-8 text-center animate-in fade-in zoom-in-95 duration-500">
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600 max-w-sm mx-auto">
                  <strong>{form.name}</strong> has been registered. An admin will review and verify your profile. In the meantime, you can explore the platform.
                </p>
              </div>
              <div className="rounded-2xl border border-amber-100 bg-amber-50 px-5 py-4 text-sm text-amber-800">
                <strong>Status: Pending Review</strong> — You'll be able to list products once your company is verified.
              </div>
              <button
                onClick={() => navigate('/dashboard')}
