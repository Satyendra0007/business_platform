...
import {
  ...
  ArrowRight, Clock, Package, Truck, Route, ShieldCheck, DollarSign
} from 'lucide-react';
...
function DealCard({ deal, navigate, isShippingAgent }) {
  ...
  const metrics = isShippingAgent
    ? [
        { label: 'Quantity', value: deal.quantity ? String(deal.quantity) : '-', icon: Package },
        { label: 'Route', value: route, icon: Route },
        { label: 'Incoterm', value: deal.incoterm || '-', icon: Truck },
        { label: 'Shipment', value: deal.shipment?.status?.replace(/_/g, ' ') || 'not started', icon: ShieldCheck },
      ]
    : [
        { label: 'Quantity', value: deal.quantity ? String(deal.quantity) : '-', icon: Package },
    ...
    <article className="overflow-hidden rounded-[28px] border border-[#d8e2ef] bg-white shadow-[0_18px_48px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(15,23,42,0.10)]">
      ...
      <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-6 py-4">
        <p className="text-xs text-slate-400">
          {isShippingAgent
            ? 'Assigned logistics workspace only. Review cargo lane details and keep shipment milestones updated.'
...
function EmptyState({ navigate, isShippingAgent }) {
  return (
    <div className="flex flex-col items-center gap-5 rounded-[28px] border border-dashed border-slate-200 py-24 text-center">
      ...
      <div>
        ...
        <p className="mt-2 max-w-sm text-sm text-slate-500">
          {isShippingAgent
            ? 'Awarded shipment workspaces will appear here once a bid is accepted for you.'
    ...
    <AppShell
      ...
      subtitle={
        isShippingAgent
          ...
          : 'Shared workspaces where buyer, supplier, and logistics teams coordinate from inquiry through shipment delivery.'
      }
    >
      <div className="space-y-5">
        ...
        {loading ? (
          ...
          <EmptyState navigate={navigate} isShippingAgent={isShippingAgent} />
        ) : (
          <div className="space-y-4">
            {deals.map((deal) => (
              <DealCard key={deal._id} deal={deal} navigate={navigate} isShippingAgent={isShippingAgent} />
            ))}
          </div>
        )}
